...
export default function WaiterOrdersScreen() {
  const { token, ordersReadyCount, decrementOrderBadge, syncOrderBadge } =
    useWaiter();
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (syncOrderBadge) {
        syncOrderBadge(activeOrders.filter((o) => o.status === "ready").length);
      }
    } catch (error: any) {
  ...
  const renderOrderItem = ({ item }: { item: any }) => {
    ...
    const displayLocation = item.table_number || "Takeaway";
    const displayCustomer = item.customer_name || "Guest";
    ...
    return (
      <View
        style={[
          ...
          !isReadyToServe && styles.cardMuted,
        ]}
      >
        <View style={styles.cardHeader}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
            ...
            >
              <Text style={styles.tableBadgeText}>{displayLocation}</Text>
            </View>
            <View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 26, fontWeight: "900", color: ANN.darkBlue },
  headerSubtitle: {
    fontSize: 14,
    color: "#64748b",
    marginTop: 2,
  ...
  orderIdText: { fontSize: 14, fontWeight: "800", color: "#1e293b" },
  ...
  itemName: { fontSize: 16, fontWeight: "700", color: ANN.darkBlue },
  itemNote: {
    fontSize: 13,
    color: ANN.orange,
    fontStyle: "italic",
