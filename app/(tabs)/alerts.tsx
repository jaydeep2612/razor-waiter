...
export default function WaiterAlertsScreen() {
  const { waiter, lastAlertPayload, resetAlertBadge } = useWaiter(); // 👈 Added resetAlertBadge
  ...
  const handleResolveAlert = (id: string) => {
    ...
    setAlerts((prev) => {
      ...
      // 👇 BUG 2 FIXED: Wipe badge if all alerts are cleared
      if (updated.length === 0) resetAlertBadge();
      return updated;
    });
...
const styles = StyleSheet.create({
  ...
  alertIconBoxUrgent: { backgroundColor: "rgba(241, 107, 63, 0.15)" },
  alertContent: { flex: 1 },
  alertTitle: {
    fontSize: 16,
    fontWeight: "bold",
