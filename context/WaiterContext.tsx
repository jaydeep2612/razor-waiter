...
interface WaiterContextType {
  ...
  decrementOrderBadge: () => void;
  syncOrderBadge: (count: number) => void;
}
...
export function WaiterProvider({ children }: { children: React.ReactNode }) {
  ...
  const resetAlertBadge = () => setAlertsCount(0);
  const decrementOrderBadge = () =>
    setOrdersReadyCount((prev) => Math.max(0, prev - 1));
  const syncOrderBadge = (count: number) => setOrdersReadyCount(count);
